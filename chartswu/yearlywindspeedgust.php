...
<html>
	<head>
		...
		<title>OUTDOOR WIND DATABASE CHART</title>	
	...
	function processData1(allText) {
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				...
				if ( rowData.length >1)
					dataPoints1.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM"),y:parseFloat(rowData[12]*<?php echo $windconv;?>)});
			}
		}
		...
		if(allLinesArray.length>0){
			...
			for (var i = 2; i <= allLinesArray.length-1; i++) {
				var rowData = allLinesArray[i].split(',');
				if ( rowData.length >1)
					dataPoints2.push({label:moment(rowData[0],'YYYY-MM-DD').format("MMM Do"),y:parseFloat(rowData[14]*<?php echo $windconv;?>)});
		...
		axisY:{
		title: "<?php echo $lang['Windspeed'] ;?> - <?php echo $lang['Gust'] ;?> (<?php echo $windunit ;?>) ",
		titleFontColor: "#ccc",
		titleFontSize: 10,
        titleWrap: false,
		margin: 10,
...
<body>
<div class="weather34darkbrowser" url="<?php echo $stationlocation;?> <?php echo $lang['Windspeed'] ;?> - <?php echo $lang['Gust'] ;?> (<?php echo $windunit ;?>) <?php echo date('Y') ;?>"></div>
